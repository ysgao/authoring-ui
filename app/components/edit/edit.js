...
// all task editing functionality
  ...
  .controller('EditCtrl', function EditCtrl($scope, $rootScope, $location, $modal, layoutHandler, metadataService, accountService, scaService, inactivationService, terminologyServerService, componentAuthoringUtil, notificationService, $routeParams, $timeout, $q, crsService, reviewService, ngTableParams, templateService, $filter, hotkeys, modalService, permissionService, vsCodeService) {
    ...
    $scope.setView = function (name, skipLoadingEditPanelConcepts) {
      ...
      switch (name) {
        case 'validation':
          if ($scope.role === 'REVIEWER_ONLY') {
            $rootScope.pageTitle = 'Validation/' + $scope.codeSystemShortname + '/' + $routeParams.projectKey + '/' + $routeParams.taskKey;
          } else {
        ...
        case 'inactivation':
          if ($scope.role === 'REVIEWER_ONLY') {
            $rootScope.pageTitle = 'Inactivation/' + $scope.codeSystemShortname + '/' + $routeParams.projectKey + '/' + $routeParams.taskKey;
          } else {
        ...
        case 'feedback':
          if ($scope.role === 'REVIEWER_ONLY') {
            $rootScope.pageTitle = 'Providing Feedback/' + $scope.codeSystemShortname + '/' + $routeParams.projectKey + '/' + $routeParams.taskKey;
          } else {
        ...
        case 'classification':
          if ($scope.role === 'REVIEWER_ONLY') {
            $rootScope.pageTitle = 'Classification/' + $scope.codeSystemShortname + '/' + $routeParams.projectKey + '/' + $routeParams.taskKey;
          } else {
        ...
        case 'edit-default':
          ...
          if ($scope.role === 'REVIEWER_ONLY') {
            $rootScope.pageTitle = 'Edit Concepts/' + $scope.codeSystemShortname + '/' + $routeParams.projectKey + '/' + $routeParams.taskKey;
          } else {
        ...
        case 'batch':
          if ($scope.role === 'REVIEWER_ONLY') {
            $rootScope.pageTitle = 'Batch Concepts/' + $scope.codeSystemShortname + '/' + $routeParams.projectKey + '/' + $routeParams.taskKey;
          } else {
